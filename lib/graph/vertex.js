 ...
 */
get upstream() {
	return _.uniq(this._uplinks.reduce((prev, cur) => {
		return [ ...prev, cur.from, ...cur.from.upstream ];
	}, [], this));
}
 ...
 */
get downstream() {
	return _.uniq(this._downlinks.reduce((prev, cur) => {
		return [ ...prev, cur.to, ...cur.to.downstream ];
	}, [], this));
}
 ...
 */
reflow() {
	var pointer = this, len = this._downlinks.length;
	for (let i = 0; i < len; i++) {
...
connect(target) {
	...
	if (this.isAfter(target)) {
		this.insertAfter(target.remove());
	}
